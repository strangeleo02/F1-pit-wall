...
async def _orchestrate_retrieval_and_synthesis(
    ...
    # 1. Fetch Telemetry conditionally
    telemetry_data = {}
    if intent != QueryIntent.RADIO_ONLY:
        try:
            telemetry_data = await get_telemetry_async(
                year=request.year,
    ...
    # 2. Search Vector DB conditionally with On-Demand Auto-Ingestion
    radio_context = []
    if intent != QueryIntent.TELEMETRY_ONLY:
        try:
